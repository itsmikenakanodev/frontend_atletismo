<template>
  ...
  <div class="card" v-if="usuarios">
    ...
    <Dialog v-model:visible="documentosVisible" :modal="true" :style="{ width: '75vw' }">
      <TablaDocumentos :docs="documentosUsuarios" :correo="usuarioCorreo" :id="usuarioId"
        @cambioEstado="(val) => cambioEstado(val)"></TablaDocumentos>
    </Dialog>
...
export default {
  ...
  methods: {
    ...
    verDocumentos(data) {
      ...
      //this.documentosUsuarios=data.documentosUsuarios
    ...
    async getUsuarios() {
      ...
      //await getUsuariosFachada().then((response) => this.usuarios=response);
    },
    getValue(slotProps) {
