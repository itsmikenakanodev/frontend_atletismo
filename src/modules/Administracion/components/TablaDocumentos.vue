...
import {
  actualizarEstadoUsuarioFachada,
  eliminarUsuarioFachada,
} from "../helpers/actualizarEstadoUsuario";
import { storage } from "@/modules/Registro/helpers/firebase";
import { eliminarDocumentoRegistroFachada } from "../helpers/getUsuarios";
    ...
    async enviarCorreo(val) {
      if (val) {
        this.cuerpoCorreo.cuerpo =
          "Tu registro ha sido aprobado. Ahora puedes iniciar sesión.";
        ...
        this.cuerpoCorreo.cuerpo =
          "Tus documentos para registro no son válidos. Intenta registrarte nuevamente!";
        this.cuerpoCorreo.mensajeHtml = this.cuerpoCorreo.mensajeHtml =
          `
          <body>
            <div style="color:rgb(144,221,240); text-align: center;">
              <img width="100%" height="100%" src="../../Inicio/aname-footer.png"/>
    ...
    cambiarEstado() {
      this.$confirm.require({
        ...
        reject: async () => {
          ...
          await eliminarDocumentoRegistroFachada(this.documentosUsuarios[0].id);
